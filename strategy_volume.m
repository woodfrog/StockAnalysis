...
if historyClose(dayIndex) >=  historyClose(dayIndex-1) %当天收盘价较前一天上涨
    ...
    if volIndex <= 4 + VOL_BUY_DAY -1
        ...
        if VOL_AVR(volIndex) >= 2 * VOL_AVR(volIndex-1) && VOL_AVR(volIndex) >= 2 * VOL_AVR(volIndex-2) && VOL_AVR(volIndex) >= 2 * VOL_AVR(volIndex-3)...
                && VOL_AVR(volIndex) >= 2 * VOL_AVR(volIndex-4)
            volBuyFlag = 1;
        else
